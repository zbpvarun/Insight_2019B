...
def update_output(n_clicks, zipc, dist, dist_wt, reco_wt, mort_wt, compli_wt):
    if n_clicks is not None:
      ...
      hosp_score = reduced_hosps['Final Score'].iloc[0]
      return u'''
          The ZIP code entered was {},
          Distance limit was {},
          and the weights for Distance, Recommendation scores, Mortality and complications were {}, {}, {}, {} \n
          \n
          For these values, the recommended hospital is {} with a composite score of {:.2f}

      '''.format(zipc, int(dist), dist_wt, reco_wt, mort_wt, compli_wt, hosp_name, hosp_score)
